...
function ImportModal({ eventId, onClose, onSaved }) {
  ...
  const parseCSV = (text) => {
    ...
    // Parse header row
    const { fields: rawHeaders, next: dataStart } = tokeniseRow(normalised, 0);
    ...
    while (pos < normalised.length) {
      const { fields, next } = tokeniseRow(normalised, pos);
      pos = next;
      ...
      if (obj.gender) {
        obj.gender = obj.gender.trim().charAt(0).toUpperCase() + obj.gender.trim().slice(1).toLowerCase();
      }
      result.push(obj);
    }
    return result;
  ...
  const onFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const errs = [];
      parsed.forEach((r, i) => {
        const fn = r.first_name || r.firstname || r.firstName || '';
        const sn = r.surname || '';
        if (!fn.trim())  errs.push(`Row ${i+1}: first_name is missing`);
        if (!sn.trim())  errs.push(`Row ${i+1}: surname is missing`);
        if (!['Male','Female'].includes(r.gender)) {
          errs.push(`Row ${i+1}: gender must be Male or Female (got "${r.gender}")`);
        }
      });
      setErrors(errs);
    };
  ...
  const doImport = async () => {
    ...
    try {
      const attendees = rows.map(r => ({
        firstName:  (r.first_name || r.firstname || r.firstName || '').trim(),
        surname:    (r.surname || '').trim(),
        gender:     r.gender,
        // accept fellowship OR church_center column
        fellowship: (r.fellowship || r.church_center || r.churchcenter || '').trim(),
        phone:      (r.phone   || '').trim(),
        email:      (r.email   || '').trim(),
  ...
  return (
    <Modal title="Import CSV" onClose={onClose}
      footer={<>
        <button className="btn btn-ghost btn-sm" onClick={onClose}>Cancel</button>
        <button className="btn btn-gold btn-sm" onClick={doImport}
      ...
      {errors.length > 0 && (
        <Alert type="danger">
          <div>{errors.slice(0, 5).map((e, i) => <div key={i}>{e}</div>)}</div>
        </Alert>
      )}
      {msg && <Alert type="danger">{msg}</Alert>}
      {rows.length > 0 && errors.length === 0 && (
        <div className="table-wrap mt-12" style={{ maxHeight: 220, overflowY: 'auto' }}>
          ...
          {rows.length > 10 && (
            <p className="text-muted text-center mt-8">…and {rows.length - 10} more rows</p>
          )}
        </div>
      )}
    </Modal>
...
function Accommodation({ activeEvent }) {
  ...
  const doDelete = async (u) => {
    const confirmed = window.confirm(`Delete "${u.name}" permanently? This action cannot be undone.`);
    if (!confirmed) return;
    try {
      await api.deleteUnit(u._id);
      reload();
            ...
            return (
              <div key={u._id} className={`unit-card ${!u.isActive?'inactive':''}`}>
                ...
                <div style={{ display:'flex', gap:7 }}>
                  ...
                  <button className="btn btn-danger btn-sm btn-icon" title="Delete" onClick={()=>doDelete(u)}><Icons.Trash /></button>
                </div>
              </div>
            );
          })}
...
function Reports({ activeEvent }) {
  ...
  const printSheet = () => {
    const rows = sheet?.data || [];
    const html = `<html><head><title>Attendance</title><style>
      body{font-family:Georgia,serif;font-size:10px;color:#111;padding:16px}
      h1{font-size:17px;margin-bottom:3px}p{color:#555;margin-bottom:10px;font-size:10px}
      table{width:100%;border-collapse:collapse}
      th,td{border:1px solid #ccc;padding:4px 7px;text-align:left}
      th{background:#f0f0f0;font-weight:600;font-size:9px;text-transform:uppercase;letter-spacing:0.05em}
      tr:nth-child(even){background:#fafafa}
